...
.file_name <- paste("CLOSE_BB_SCR_", Sys.Date(), ".RData", sep = "")
...
.exc1 <- rep(NA, .N)
.exc1 <- ifelse(nurtureu_introduction$intro_ineligible == 1, NA, "Not meeting inclusion criteria")
...
.exc1_bb[filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$randomisation_complete == "Incomplete" & difftime(.rando_dt, filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$con_start_dt, units = "days") < 15] <- "Incomplete (Awaiting data entry)"
...
.rando_dt[.arm_bb == "Arm A"] <- .rando_dt[.arm_bb == "Arm A"] + lubridate::weeks(6)
...
```{r}
#| label: Consort_bb_arms
#| output: false

.out_bb_arms <- consort_plot(data = .df_bb,
...
cat(build_grviz(.out_bb_arms), file = "consort_bb_arms.gv")

```

```{r}
#| label: Consort_bb_arms_save
#| output: false
png("consort_diagram_bb_closed.png", width = 30, 
...
```{dot}
//| label: fig-consort_bb_arms
//| fig-cap: "CONSORT diagram of NURTURE-U Bounce Back."
//| file: consort_bb_arms.gv
```
...
# The following filters out duplicate entries from the CRF data.
.pri_out <- filter(.df_bb, !is.na(.df_bb$.trialno_bb)) %>%
  filter(.arm_bb == "Arm A")
.rand_a <- select(.pri_out, ".trialno_bb")
