---
title: "Internet CBT DMEC v2 (closed)"
echo: false
---
...
.file_name <- paste("CLOSE_CBT_SCR_", Sys.Date(), ".RData", sep = "")
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("CLOSE_CBT_CBT_1_", Sys.Date(), ".RData", sep = "")
...
.exc3_cbt[2][.exc3_cbt[2] == "Incomplete"] <- NA
.exc3_cbt[2][is.na(.exc3_cbt[2]) & findInterval(difftime(Sys.Date(), .rando_dt, units = "weeks"), c(8,9)) == 2] <- "Incomplete (overdue)"
.exc3_cbt[2][is.na(.exc3_cbt[2]) & findInterval(difftime(Sys.Date(), .rando_dt, units = "weeks"), c(8,9)) == 1] <- "Incomplete (awaiting data entry)"
.exc3_cbt[2][is.na(.exc3_cbt[2]) & findInterval(difftime(Sys.Date(), .rando_dt, units = "weeks"), c(8,9)) == 0] <- "Incomplete (awaiting access)"
.exc3_cbt[2][.exc3_cbt[2] == "Complete"] <- NA
.exc3_cbt[2][is.na(.exc3_cbt[1])] <- NA
.exc3_cbt <- .exc3_cbt[2]
...
#| output: false
png("consort_diagram_cbt_closed.png", width = 30, 
    height = 40, res = 300, units = "cm", type = "cairo") 
plot(.out_cbt)
dev.off() 
```
...
.dfs_all <- Filter(function(x) is(x, "data.frame"), mget(ls())) # Creates a list of CRFs. This allows for functions to be applied to all CRFs at once if needed.
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("CLOSE_CBT_CBT_2_", Sys.Date(), ".RData", sep = "")
...
```{r}
#| label: data_filter_2_A
#| output: false

rm(list = c("i","j", "is_present"))

.pri_out <- filter(.df_cbt, !is.na(.df_cbt$.trialno_cbt)) %>%
  filter(.arm_cbt == "Arm A")
.rand_a <- select(.pri_out, ".trialno_cbt")
colnames(.rand_a) <- "record_id"
.dfs <- lapply(.dfs_all, function(x) x[x$record_id %in% .rand_a[[1]],])
...
.base_data[3][is.na(.base_data[ncol(.base_data)])] <- .trialno_cbt[is.na(.base_data[ncol(.base_data)])]
...
.wk_data <- lapply(.wk_data_all, function(x) x[x$record_id %in% .rand_a[[1]],])
...
.exp_count <- data.frame(
  ...
  count = c(34,34,34,42,42,42,42,42,42,42,42,36,36,42,42,42,42,42)/2) %>%
  mutate(cum_count = cumsum(count)) %>%
...
ggplot(.part_count_both, aes(fill=state, y=cum_count/.coeff, x=month)) + # Selects data for the graph. Note the secondary y axis data is divided by .coeff so that it will fit the modified axis.
  ...
  ggtitle("Monthly Randomisation Count\nin Internet CBT (Arm A)") # Creates title for the graph.

```

```{r}
#| label: tbl-bline_sites_cbt_A
#| tbl-cap: "Summary of Internet CBT sites open (Arm A)."
...
## Data completeness (Arm A)

### Number of overdue CRFs by CRF (Arm A)
...
### Primary Outcome Characteristics (Arm A)

```{r}
#| label: tbl-pri_out_pri_pool_A
...
### Secondary Outcome Characteristics (Arm A)

```{r}
#| label: pri_out_sec_pool_A
...
```{r}
#| label: data_filter_2_B
#| output: false

rm(list = c("i","j", "is_present"))

.pri_out <- filter(.df_cbt, !is.na(.df_cbt$.trialno_cbt)) %>%
  filter(.arm_cbt == "Arm B")
.rand_a <- select(.pri_out, ".trialno_cbt")
colnames(.rand_a) <- "record_id"
.dfs <- lapply(.dfs_all, function(x) x[x$record_id %in% .rand_a[[1]],])
names(.dfs) <- set_names(ls(all.names = FALSE))
...
ggplot(.part_count_both, aes(fill=state, y=cum_count/.coeff, x=month)) + # Selects data for the graph. Note the secondary y axis data is divided by .coeff so that it will fit the modified axis.
  ...
  ggtitle("Monthly Randomisation Count\nin Internet CBT (Arm B)") # Creates title for the graph.

```

```{r}
#| label: tbl-bline_sites_cbt_B
#| tbl-cap: "Summary of Internet CBT sites open (Arm B)."
...
### Primary Outcome Characteristics (Arm B)

```{r}
#| label: tbl-pri_out_pri_pool_B
#| tbl-cap: "Summary of Internet CBT Primary Outcome Characteristics."
...
### Secondary Outcome Characteristics (Arm B)

```{r}
#| label: pri_out_sec_pool_B
