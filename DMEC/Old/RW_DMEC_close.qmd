---
title: "Reducing Worry DMEC (Closed)"
echo: false
---
...
.file_name <- paste("CLOSE_WOR_SCR_", Sys.Date(), ".RData", sep = "")
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("CLOSE_WOR_WOR_1_", Sys.Date(), ".RData", sep = "")
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("CLOSE_WOR_WOR_2_", Sys.Date(), ".RData", sep = "")
...
.pri_out <- filter(.df_wor, !is.na(.df_wor$.trialno_wor)) %>%
  filter(.arm_wor == "Arm A")
.rand_a <- select(.pri_out, ".trialno_wor")
...
.exp_count <- data.frame(
  ...
  count = c(59,59,59,59,59,59,59,59,59,59,58)/2) %>%
  mutate(cum_count = cumsum(count)) %>%
...
ggplot(.part_count_both, aes(fill=state, y=cum_count/.coeff, x=month)) + # Selects data for the graph. Note the secondary y axis data is divided by .coeff so that it will fit the modified axis.
  ...
  ggtitle("Monthly Randomisation Count\nin Reducing Worry (Arm A)")

```

```{r}
#| label: tbl-bline_sites_wor_A
...
ggplot(.part_count_both, aes(fill=state, y=cum_count/.coeff, x=month)) + # Selects data for the graph. Note the secondary y axis data is divided by .coeff so that it will fit the modified axis.
  ...
  ggtitle("Monthly Randomisation Count\nin Reducing Worry (Arm B)")

```

```{r}
#| label: tbl-bline_sites_wor_B
#| tbl-cap: "Summary of Internet wor sites open."
