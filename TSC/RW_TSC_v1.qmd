---
title: "Reducing Worry TSC Report"
date-modified: today
format: html
...
.file_name <- paste("TSC_SCR_", "2023-11-17", ".RData", sep = "")
...
# This loads data from file. If ran just after importing data that should just be the CRFs from REDCap.
rm(list = ls())
load(paste(.file_path, .file_name, sep=""))
##------------------------------------------------------------------------------

.test <- CTUReportTool::ct_by_var(col_data = eligibility,
                         row_data = consent_for_bounce_back_trial,
                         col_var = eli_university,
                         row_var = consent_for_bounce_back_trial_complete)
...
.lidas <- rep(NA, .N)
.lidas[!is.na(.induc3)] <- lidas_adapted$lidas_excluded[!is.na(.induc3)]
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("TSC_BB_", "2023-11-17", ".RData", sep = "")
...
.exc1_bb <- rep(NA, .N)
.exc1_bb[filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$randomisation_complete == "Incomplete" & difftime(.rando_dt, filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$con_start_dt, units = "days") > 14] <- "Over timepoint window and blocked"
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("TSC_CBT_1_", "2023-11-17", ".RData", sep = "")
...
.exc1_cbt <- rep(NA, .N)
.exc1_cbt[filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$randomisation_complete == "Incomplete" & difftime(.rando_dt, filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$cont_start_dt, units = "days") > 14] <- "Over timepoint window and blocked"
...
### Variables
# Replace with the desired filename, date should be included in filename.
.file_name <- paste("TSC_WOR_", "2023-11-17", ".RData", sep = "")
...
# Finds participants who have not completed baseline and their current status.
.rando_dt <- drop_all_labels(filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$rando_dt) 
.rando_dt[is.na(.rando_dt)] <- "2023-11-17"
#.rando_dt[is.na(.rando_dt)] <- Sys.Date()


.exc1_wor <- rep(NA, .N)
.exc1_wor[filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$randomisation_complete == "Incomplete" & difftime(.rando_dt, filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$con_start_dt, units = "days") > 14] <- "Over timepoint window and blocked"
...
.exc1_wor[grepl("DUPLICATED", filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$record_id, ignore.case = TRUE)] <- "Duplicate participant*"
...
# Identifies which arm each participant has been allocated to.
.arm_wor <- rep(NA, .N)
.arm_wor[!is.na(.induc1_wor)] <- filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$rando_allocation[!is.na(.induc1_wor)]
.arm_wor[!is.na(.induc1_wor)] <- ifelse(grepl(.arm_a,.arm_wor[!is.na(.arm_wor)]), "Arm A", "Arm B")[!is.na(.induc1_wor)]
...
.out_wor <- consort_plot(data = .df_wor,
                    orders = c(.induc3 = "Consented to Consent Form B",
                               ...
                               .induc1_wor = "Participant Randomised",
...
#| output: false
png("consort_diagram_scr_tsc_2.png", width = 40, 
...
#| output: false
png("consort_diagram_cbt_tsc_2.png", width = 30, 
    height = 20, res = 300, units = "cm", type = "cairo") 
plot(.out_cbt)
dev.off() 
```
...
#| output: false
png("consort_diagram_bb_tsc_2.png", width = 30, 
    height = 20, res = 300, units = "cm", type = "cairo") 
plot(.out_bb)
dev.off() 
```
...
#| output: false
png("consort_diagram_wor_tsc_2.png", width = 30, 
    height = 20, res = 300, units = "cm", type = "cairo") 
plot(.out_wor)
dev.off() 
```
...
# The following filters out duplicate entries from the CRF data.
.dfs_all <- .dfs %>% 
  lapply(function(x) filter(x,!grepl("DUPLICATED", record_id, ignore.case = TRUE)))
names(.dfs_all) <- set_names(ls(all.names = FALSE))
list2env(.dfs_all, envir = .GlobalEnv)
...
.scr_id <- filter(.scr_id,!grepl("DUPLICATED", .trialno_wor, ignore.case = TRUE))
...
.base_data[3][is.na(.base_data[ncol(.base_data)])] <- .scr_id$.trialno_wor[is.na(.base_data[ncol(.base_data)])]
...
.fu_3m_dfs <- list(lidas_adapted_3_month_fu, phq9_fu, risk_contact_fu, gad7_fu, item_brooding_scale_fu, shortform_penn_state_worry_questionnaires_fu, wemwbs, wsas, brief_resilience_scale, academic_grades_selfreport, perceived_stress_scale, pssi, negative_experience_measure, use_of_services_treatment_received_nhs)

.trialno_wor <- filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$record_id
.trialno_wor <- as.data.frame(.trialno_wor)
colnames(.trialno_wor) <- "record_id"

.fu_3m_data <- mutate(.trialno_wor, .fu_3m_comp = NA)
...
.fu_3m_data[2][is.na(.fu_3m_data[ncol(.fu_3m_data)])] <- .scr_id$.trialno_wor[is.na(.fu_3m_data[ncol(.fu_3m_data)])]
...
#| fig-cap: "Summary of Reducing Worry randomisation (Actual vs Expected)."

CTUReportTool::ct_recruit_timeplot(record_data = filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE)),
                                   date_var = rando_dt,
                                   date_grouping = "month",
                                   exp_start = "2023/07/01",
                                   exp_recruitment = c(59,59,59,59,59,59,59,59,59,59,58),
...
.record_data <- merge(filter(randomisation, grepl("Baseline", redcap_event_name, ignore.case = TRUE)), filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE)), by = "record_id")
...
.pri_out <- filter(.df_wor, !is.na(.df_wor$.trialno_wor)) %>%
    filter(!grepl("DUPLICATED", .trialno_wor, ignore.case = TRUE))

.phq_out_table <- data.frame(time = c("PHQ-9 (Baseline)", "PHQ-9 (3 Month FU)"),
                             num_exp = c(nrow(.pri_out), .fu_3m_comp[2,2]),
                             num_over = c((nrow(filter(.pri_out, .pri_out$.exc1_wor == "(Blocked)"))), .fu_3m_comp[2,3])
                             )

.gad_out_table <- data.frame(time = c("GAD-7 (Baseline)", "GAD-7 (3 Month FU)"),
                             num_exp = c(nrow(.pri_out), .fu_3m_comp[4,2]),
                             num_over = c((nrow(filter(.pri_out, .pri_out$.exc1_wor == "(Blocked)"))), .fu_3m_comp[4,3])
                             )

.lidas_out_table <- data.frame(time = c("LIDAS (Baseline)", "LIDAS (3 Month FU)"),
                             num_exp = c(nrow(.pri_out), .fu_3m_comp[1,2]),
                             num_over = c((nrow(filter(.pri_out, .pri_out$.exc1_wor == "(Blocked)"))), .fu_3m_comp[1,3])
                             )

.pri_out_table <- rbind(.phq_out_table,.gad_out_table, .lidas_out_table)
...
if (nrow(.lid_count[.lid_count[1] == TRUE,]) != 0) {
...
.gender <- make_choose_one_table(filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$dem_gender)

.sex_orient <- make_choose_one_table(filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$dem_sex_orient)

.uni_stat <- make_choose_one_table(filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$dem_uni_status) 

.uni_choice <- make_choose_one_table(filter(contact_details, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$cont_uni) 

.ethnic <- make_choose_all_table(filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE)), "dem_ethnicity_")

.disability <- filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$dem_disability_txt
.disability[is.na(.disability)] <- filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE))$dem_disability[is.na(.disability)]
.disability[.disability == "No"] <- "No disability"
.disability <- make_choose_one_table(.disability) 

.diagnosis <- make_choose_all_table(filter(demographics, grepl("Baseline", redcap_event_name, ignore.case = TRUE)), "dem_diagnos___")
...
.brood_out_table <- data.frame(time = c("Brood (Baseline)", "Brood (3 Month FU)"),
                             num_exp = c(nrow(.pri_out), .fu_3m_comp[4,2]),
                             num_over = c((nrow(filter(.pri_out, .pri_out$.exc1_wor == "Incomplete form (blocked)"))), .fu_3m_comp[4,3])
                             )

.pswq_out_table <- data.frame(time = c("PSWQ (Baseline)", "PSWQ (3 Month FU)"),
                             num_exp = c(nrow(.pri_out), .fu_3m_comp[5,2]),
                             num_over = c((nrow(filter(.pri_out, .pri_out$.exc1_wor == "Incomplete form (blocked)"))), .fu_3m_comp[5,3])
                             )
...
.wemwbs_no <- .pri_out_2 %>%
  filter(wemwbs_complete == "Incomplete") %>%
  filter(.exc1_wor == "Incomplete form (blocked)") %>%
  nrow()
...
.wthdrw <- CTUReportTool::ct_event_check(withdrawal,
                              c("with_rum_ch_dt", "with_rum_type", "with_rum_timepoint", "with_rum_reason", "record_id"),
                              c("Date of Status Change", "Withdrawal type", "Withdrawal timepoint", "Reason for withdrawal", "Record ID"))

print(.wthdrw[[1]])

knitr::kable(.wthdrw[[2]])
...
.lidas_3m_count <- group_by(lidas_adapted_3_month_fu, lidas_adapt_dep == 1) %>%
  summarise(count = n()) %>%
  as.data.frame() %>%
  na.omit() %>%
  mutate(percent = (count/sum(count)) * 100)  
...
# Prints the number of withdrawals.
.ae <- CTUReportTool::ct_event_check(ae,
                              c("ae_rum_number",
                                "ae_rum_event_name",
                                "ae_rum_start_dt",
                                "ae_rum_relation"),
...
# Prints the number of withdrawals.
.sae <- CTUReportTool::ct_event_check(sae,
                              c("sae_rum_number",
                                "sae_rum_event",
                                "sae_rum_onset_dt",
                                "sae_rum_relat"),
